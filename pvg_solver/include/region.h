...
public:
	...
	void set_enlarged_mask(const cv::Mat& enlarged_mask, double scale, const BoundingBox<double>& ROI, const std::vector<std::vector<std::vector<std::pair<cv::Vec2i, cv::Vec3f>>>>& points_vector);
	...
	inline std::vector<int> crossing_index(const CPoint2f& p) const
	{
		...
		if ((int)pt[0] >= 0 && (int)pt[0] < enlarged_crossing_region.shape()[0] && (int)pt[1] >= 0 && (int)pt[1] < enlarged_crossing_region.shape()[1])
		{
			if (!end_points_mask[(int)pt[0]][(int)pt[1]].empty()) return std::vector<int>();
			else return enlarged_crossing_region[(int)pt[0]][(int)pt[1]];
		}
