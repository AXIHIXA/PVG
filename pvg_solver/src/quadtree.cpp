...
QuadTree::QuadTree(const Region& region, int region_id, const cv::Mat& laplacian, int step, bool split_edge_neighbor, const Mat edge_neighbor_mask)
	:region(region), region_id(region_id), step(step)
{
	//tree initialization
				...
				if (split_edge_neighbor)
				{
					for (int m = -2; m <= 2; ++m)
					{
						...
						for (int n = -2; n <= 2; ++n)
						{
							...
							if (p[0] >= 0 && p[0] < edge_neighbor_mask.rows && p[1] >= 0 && p[1] < edge_neighbor_mask.cols)
							{
	...
	{
		if (quadtree.max_depth() < depth) break;
		cell_width /= 2;
	...
	while (quadtree.is_valid(ite))
	{
		if (ite->type == INNER) ite->index = count++;
		++ite;
	}
	ite = quadtree.begin_leaf();
	...
	while (quadtree.is_valid(ite))
	{
		if (ite->type == BOUNDARY) ite->index = count++;
		++ite;
	}
...
bool QuadTree::to_split(const TreeNodeD& node, const Mat& mask, const Mat& pseudo_laplacian, const CPoint2i& mask_ori)
{
			...
			if (p[0] >= 0 && p[1] >= 0 && p[0] < mask.rows && p[1] < mask.cols)
			{
				if (mask.at<uchar>(p[0], p[1]) != 0)
				{
					if (stand == Vec3f(-2e10, -2e10, -2e10))
					{
						...
						if (stand == Vec3f(-1e10, -1e10, -1e10)) return true;
					}
					else if (pseudo_laplacian.at<Vec3f>(p[0], p[1]) != stand) return true;
				}
			}
		}
	}
...
int QuadTree::search(const CPoint2f& p) const
{
	...
	if (ind_row < 0 || ind_row >= height || ind_col < 0 || ind_col >= width) return -1;
...
inline int find(const vector<tuple<const GEOM_FADE2D::Point2*, const GEOM_FADE2D::Point2*, const GEOM_FADE2D::Point2*>>& v, const GEOM_FADE2D::Point2* ptr)
{
	for (size_t i = 0; i < v.size(); ++i)
	{
		if (get<0>(v[i]) == ptr) return int(i);
	}
	return -1;
}
...
void QuadTree::construct_laplacian()
{
	...
	vector<vector<tuple<const GEOM_FADE2D::Point2*, const GEOM_FADE2D::Point2*, const GEOM_FADE2D::Point2*>>> adjacent_list(pixel_node_count);
	...
	for (size_t i = 0; i < adjacent_list.size(); ++i)
	{
		...
		for (size_t j = 0; j < adjacent_list[i].size(); ++j)
		{
			...
			triplet_list_basis.push_back(Eigen::Triplet<double>(points_list[get<0>(adjacent_list[i][j])->getCustomIndex()]->index, (int)i, d1));
			total_basis += d1;
			...
			{
				total_solver += d1;
				if (r < inner_node_count) triplet_list_solver.push_back(Eigen::Triplet<double>(r, (int)i, -d1));
			}
		...
		if (i < (size_t)inner_node_count) triplet_list_solver.push_back(Eigen::Triplet<double>((int)i, (int)i, total_solver));
	}
...
void QuadTree::get_regions(vector<TreeNodeD>& regions) const
{
	...
	while (quadtree.is_valid(ite))
	{
		if (ite->index >= 0) regions[ite->index] = *ite;
...
{
	if (p[0] >= original[0] && p[1] >= original[1] && p[0] < original[0] + step*height && p[1] < original[1] + step*width)
		return true;
	else
		return false;
}
