...
AdaptiveEvaluation::AdaptiveEvaluation(const QuadTree& quadtree, const std::vector<cv::Vec3f>& coefs)
	:quadtree(quadtree), coefs(coefs), laplacian_matrix(quadtree.get_laplacian_basis())
{
	...
	//debug code
	{
		for (int i = 0; i < laplacian_matrix.cols(); ++i)
		{
			...
			vector<float> value;
			double d = 0;
			vector<CPoint2d> knots;
			for (Eigen::SparseMatrix<double>::InnerIterator it(laplacian_matrix, i); it; ++it)
			{
				value.push_back(it.value());
				index.push_back(it.row());
				d += norm(regions[i].center() - regions[it.row()].center());
				knots.push_back(regions[it.row()].center());
				printf("(%f %f) ", norm(regions[i].center() - regions[it.row()].center()), regions[it.row()].width);
			}
			printf("%f\n", d / (index.size() - 1));
			save_basis(regions[i].center(), index, value, regions, knots);
		}
	}
	//end debug
#endif
...
double source_double(const CPoint2d& func, double pt_x, double pt_y)
{
	...
	if (x == 0.0 || y == 0.0)  v = y2 = 0.0;

	return v1 > 0 ? v + y2 : v - y2;
}

double green_integral_double(const CPoint2d& func, double x1, double x2, double y1, double y2)
{
	return source_double(func, x1, y1) + source_double(func, x2, y2) - source_double(func, x1, y2) - source_double(func, x2, y1);
}

vector<CPoint2i> AdaptiveEvaluation::full_solution(const Region& region, const BoundingBox<int>& rect, double step, const CPoint2d& original, const cv::Mat& laplacian_image, int n_ring, cv::Mat& result)
{
		...
		for (int j = 0; j < rect.width; ++j)
		{
			...
			if(index[i*rect.width + j] == 0)
			{
				...
				if (id >= 0 && id < quadtree.get_number_of_inner_points())
				{
					...
					{
						val += coef[k] * green_integral_double(pt, nodes[k].row, nodes[k].row + nodes[k].width, nodes[k].col, nodes[k].col + nodes[k].width) / (nodes[k].width*nodes[k].width);
					}
					val *= 0.25 / CV_PI;
