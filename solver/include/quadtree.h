...
private:
	...
	inline bool to_split(const TreeNodeD& node, const cv::Mat& mask, const cv::Mat& pseudo_laplacian,const CPoint2i& mask_ori);
	void insert(std::vector<int>& neighbors, int row, int col, const CPoint2d& pt, double radius) const;

public:
	//step must be 2^n
	QuadTree(const Region& region, int region_id, const cv::Mat& laplacian, int step, bool split_edge_neighbor, const cv::Mat edge_neighbor_mask);
