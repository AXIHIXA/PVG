...
public:
	...
	~AdaptiveEvaluation();
	//CUDA functions
	std::vector<CPoint2i> cuda_evaluation(const Region& region, const BoundingBox<int>& rect, double step, const CPoint2d& original, const cv::Mat& laplacian_image, int n_ring, cv::Mat& result);
	static void jacobian(cv::Mat& result, const cv::Mat& enlarged_side_mask, std::vector<CPoint2i>& points, int round, int radius);

	std::vector<CPoint2i> full_solution(const Region& region, const BoundingBox<int>& rect, double step, const CPoint2d& original, const cv::Mat& laplacian_image, int n_ring, cv::Mat& result);
};
