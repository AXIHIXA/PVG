...
void PoissonSolver::region_computation(int index, bool recompute, const BoundingBox<double> & box, bool simple_solver,
                                       int n_rings)
{
#ifdef OUTPUTLOG
    clock_t t = clock();
    ...
    if (recompute)
    {
        trees[index].reset(new QuadTree(region, index, laplacian_image, 8, simple_solver, convert_to_laplacian_mask));
    }
...
#else
        critical_points[index] = evaluators[index]->cuda_evaluation(region, box_, 1.0 / scale,
                                                                    CPoint2d(box_.row / scale + original[0],
                                                                             box_.col / scale + original[1]),
                                                                    laplacian_image, n_rings, result);
#endif
    }
...
PoissonSolver::PoissonSolver(bool recompute, const Size & canvas_size, const Mat & laplacian_image,
                             const Region & region, const double scale, const CPoint2d & original,
                             const vector<CPoint2d> & end_points, bool simple_solver, int n_rings,
                             const Mat & convert_to_laplacian_mask)
        : laplacian_image(laplacian_image), region(region), scale(scale), original(original),
          critical_points(region.get_number_of_regions()), convert_to_laplacian_mask(convert_to_laplacian_mask)
{
                ...
                for (int i = regions[k].row; i < regions[k].row + regions[k].width; ++i)
                {
                    for (int j = regions[k].col; j < regions[k].col + regions[k].width; ++j)
                    {
                        if (i >= 0 && i < result.rows && j >= 0 && j < result.cols)
                        {
                            ...
                            {
                                result.at<Vec3f>(i, j) = coefs[index][k];
                            }
                        }
                    }
                }
